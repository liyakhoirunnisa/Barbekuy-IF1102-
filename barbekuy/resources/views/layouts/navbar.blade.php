@php
  ...
  // ✅ Tampilkan search di Menu, Riwayat, dan Keranjang
  $onMenu      = request()->routeIs('menu');
  $onRiwayat   = request()->routeIs('riwayat.semua') || request()->routeIs('pemesanan.riwayat');
  $onKeranjang = request()->routeIs('keranjang.index');
  $showSearch  = $onMenu || $onRiwayat || $onKeranjang;

  // ✅ Arahkan action sesuai halaman aktif
  if ($onMenu) {
    $searchAction = route('menu');
  } elseif ($onRiwayat) {
    $searchAction = route('riwayat.semua');
  } elseif ($onKeranjang) {
    $searchAction = route('keranjang.index');
  } else {
    $searchAction = '#';
  }
@endphp
...
<style>
  nav.navbar { background-color:#ffffff; box-shadow:0 2px 8px rgba(0,0,0,0.1); }
  ...
  .btn-login { background-color:#800000; color:#fff !important; border-radius:8px; padding:6px 18px; transition:.3s; }
  .btn-login:hover { background-color:#a00000; }

  /* === SEARCH BAR STYLE === */
  .search-box{ display:flex; align-items:center; background:#fff; border-radius:12px; padding:8px 14px;
    box-shadow:0 0 5px rgba(128,0,0,.08); border:1px solid rgba(128,0,0,.1); width:400px; transition:all .3s ease; margin:0 20px; }
  ...
  .nav-icon,.nav-keranjang{ color:#751A25; font-size:1.35rem; transition:.3s; margin-left:10px; position:relative; }
...
<nav class="navbar navbar-expand-lg navbar-light sticky-top">
  <div class="container">
    ...
    {{-- 🔍 SEARCH BAR (desktop) → hanya tampil di Menu & Riwayat --}}
    @if($showSearch)
      <form class="d-none d-lg-block" action="{{ $searchAction }}" method="GET">
        <div class="search-box d-flex align-items-center bg-white rounded-3 px-3 py-2 shadow-sm border" style="width: 400px;">
          ...
          <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari..." class="border-0 outline-none w-100" style="font-size:.95rem;color:#000;">
        </div>
      </form>
    @endif
    ...
    <div class="collapse navbar-collapse justify-content-end" id="navbarNav">
            ...
            <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="profileMenu">
              <li>
                <a class="dropdown-item" href="{{ url('/pengaturan') }}">
                  <span class="icon me-2" aria-hidden="true"><i class="bi bi-gear fs-5" style="color:#751A25;"></i></span>
                  Pengaturan Akun
                </a>
              </li>
              ...
              <li>
                <form method="POST" action="{{ route('logout') }}">
                  ...
                  <button type="submit" class="dropdown-item">
                    <span class="icon me-2" aria-hidden="true"><i class="bi bi-box-arrow-right fs-5" style="color:#751A25;"></i></span>
                    Keluar
                  </button>
                </form>
              </li>
      ...
      {{-- 🔍 SEARCH HP → hanya tampil di Menu & Riwayat --}}
      @if($showSearch)
        <form class="d-lg-none mt-3 w-100" action="{{ $searchAction }}" method="GET">
