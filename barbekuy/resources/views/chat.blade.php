...
<head>
  ...
  <style>
    ...
    .scroll-slim { -webkit-overflow-scrolling: touch; }
  </style>
</head>
...
<body class="bg-gray-100 min-h-screen overflow-hidden">
  ...
  <!-- MAIN CONTENT -->
  <main class="max-w-6xl mx-auto px-4 py-6 h-[calc(100vh-4rem)] min-h-0">
    <div class="bg-white rounded-2xl shadow overflow-hidden h-full flex flex-col min-h-0">
      ...
      <div class="px-6 py-4 border-b flex items-center gap-3 shrink-0">
        ...
        <div>
          ...
          <div class="text-xs text-gray-500">{{ now()->translatedFormat('d F') }}</div>
        </div>
      </div>

      <!-- AREA PESAN (scrollable) -->
      <div class="bg-gray-50 p-6 space-y-3 flex-1 min-h-0 overflow-y-auto scroll-slim">
        <!-- CUSTOMER -->
      ...
      <div class="p-5 border-t bg-gray-50 shrink-0">
        <form method="POST" action="{{ route('chat.send') }}" class="flex items-center gap-3">
          @csrf
